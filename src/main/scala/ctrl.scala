...
class CtrlModule()(implicit val p: Parameters) extends Module
  ...
  val stream_cnt     = RegInit(0.U(log2Ceil(max_streams).W))   // number of streams
  val rec_stream_cnt = RegInit(0.U(log2Ceil(max_streams).W))   // streams received
  ...
  val s_idx       = RegInit(0.U(log2Ceil(max_streams).W))
  ...
  switch (ctrl_state) {
    ...
    is (ctrl_access.asUInt) {
      printf(p"ctrl_access state dmem_req.fire: ${io.dmem_req.fire} s_idx: ${s_idx} s_sent: ${s_sent(s_idx)} s_rec: ${s_rec(s_idx)} s_sent_done: ${s_sent_done(s_idx)} s_rec_done: ${s_rec_done(s_idx)}\n")

      io.dmem_req.valid     := !s_sent_done(s_idx)
      io.dmem_req.bits.addr := start_addr(s_idx) + stride(s_idx) * s_sent(s_idx)
      io.dmem_req.bits.tag  := max_reqs * s_idx + s_sent(s_idx)
      io.dmem_req.bits.cmd  := Mux(stream_type === stream_rd.asUInt, M_XRD, M_XWR)
      io.dmem_req.bits.size := log2Ceil(8).U // FIXME
      io.dmem_req.bits.data := stride(s_idx)
      ...
      when (dmem_resp_val_reg) {
        val s_rec_idx = dmem_resp_tag_reg >> OHToUInt(max_reqs)
      ...
      when (s_sent_done.zipWithIndex.map { case(s, idx) => 
              val x = WireInit(true.B)
              when (idx.U < stream_cnt) {
                x := s
              }
              (x, idx)
              }.reduce{(x, y) => (x._1 && y._1, 0)}._1) {
    ...
    is (ctrl_accesspend.asUInt) {
      ...
      when (s_rec_done.zipWithIndex.map { case(r, idx) => 
              val x = WireInit(true.B)
              when (idx.U < stream_cnt) {
                x := r
              }
